...
obj = "hate" # choose dataset, you can choose either "Twitter15" or "Twitter16"
...
treePath = '../hate_resource/hatedata.TD_RvNN.txt' 

trainPath = "../hate_nfold/RNNtrainSet_"+obj+str(fold)+"_tree.txt" 
testPath = "../hate_nfold/RNNtestSet_"+obj+str(fold)+"_tree.txt"
labelPath = "../hate_resource/"+obj+"_label_All.txt"
...
def str2matrix(word, MaxL): # str = word (single token)
    if word in word2vec:
        ret_vec = word2vec[word]

    else:
        ret_vec = word2vec[random.choice(list(word2vec))]

    return ret_vec,ret_vec

def loadWordVectors():
    f = open('./glove.twitter.27B.50d.txt', 'rb')

    for l in tqdm(f,total=1193514):
        line = l.decode().split()
        word = line[0]
...
def loadLabel(label, l1, l2, l3, l4):
    '''
    labelset_nonR = normal
    labelset_f = abusive
    labelset_t = hateful
    labelset_u = spam
    '''
    labelset_nonR, labelset_f, labelset_t, labelset_u = ['normal'], ['abusive'], ['hateful'], ['spam']
...
def loadData():
    ...
    treeDic = {}
    for line in tqdm(open(treePath),total=1380537):
        line = line.rstrip()
    ...
    l1,l2,l3,l4 = 0,0,0,0
    for eid in tqdm(open(trainPath),total=70000):
        #if c > 8: break
        eid = eid.rstrip()
    ...
    l1,l2,l3,l4 = 0,0,0,0
    for eid in tqdm(open(testPath),total=20000):
        #if c > 4: break
        eid = eid.rstrip()
        if not labelDic.has_key(eid): continue
        if not treeDic.has_key(eid): continue 
